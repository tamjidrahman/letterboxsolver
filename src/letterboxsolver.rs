...
impl LetterBoxSolver {
    ...
    fn generate_next_states_from_state(&self, words_used: &Vec<Word>, letters_remaining: &HashSet<char>, starting_letter: Option<char>) -> Vec<(Vec<Word>, HashSet<char>, Option<char>)>{
        ...
        let allowed_next_words = self.dictionary
            .iter()
...
mod tests {
    ...
    fn generate_word_sequences_from_stage(stage: &Vec<(Vec<Word>, HashSet<char>, Option<char>)>) -> Vec<Vec<String>>{
        return stage.clone().into_iter().map(generate_word_sequence_from_state).collect();
    }

    fn generate_word_sequence_from_state(state: (Vec<Word>, HashSet<char>, Option<char>)) -> Vec<String>{
   ...
   fn test_generate_next_stage() {
        ...
        let dict1: Vec<Word> = vec!["dog", "cate", "catfish", "dogc"].iter().map(|&s| Word::new(s.to_string())).collect();
        let sides1: Vec<Vec<char>> = vec![
            vec!['d', 'a', 's'],
            vec!['o', 't', 'h'],
            vec!['g', 'e', 'i'],
        ...
        let states: Vec<(Vec<Word>, HashSet<char>, Option<char>)> = vec![(vec![], letters_remaining, None)];

        let first_stage = solver1.generate_next_stage(states);
        assert_eq!(generate_word_sequences_from_stage(&first_stage), vec![vec!["dog"], vec!["cate"], vec!["catfish"], vec!["dogc"]]);

        let second_stage: Vec<(Vec<Word>, HashSet<char>, Option<char>)> = solver1.generate_next_stage(first_stage);
        assert_eq!(generate_word_sequences_from_stage(&second_stage), vec![vec!["dogc", "cate"], vec!["dogc", "catfish"]]);
